...
# Rewriting the functions here because the package by default only computes PDPs on training data
# data, but we want to compute the PDPs on both the training and the testing data

compute_quantile_PDP_1D <- function(object, tau, X_data, var_name, grid_points=50, extents=NULL, verbose=FALSE){
  ...
  if (verbose) {
    cat(paste0("\nComputing 1D PDP for feature: ", var_name, "\n"))
...
compute_quantile_PDP_2D <- function(object, tau, X_data, var_names, grid_points=50, extents=NULL, verbose=FALSE) {
    ...
    # Initialize the progress bar
    if (verbose) {
      cat(paste0("\nComputing 2D PDP for features: ", var_names[1], " & ", var_names[2], "\n"))
...
permutation_score <- function(object, X_data, y_data, n_reps=500, verbose=FALSE) {
  ...
  for (i in 1:length(cols)) {
    ...
    if (verbose) {
      cat(paste0("\nProcessing feature (", i, "/", length(cols), "): ", feature, "\n"))
      # Initialize the progress bar
    ...
    if (verbose) {
      # Close the progress bar once the inner loop finishes
      close(pb)
    }
